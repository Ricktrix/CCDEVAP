...
function calculateTotalPrice(pricing) {
    ...
    let baseFare         = isNonNegativeNumber(pricing.baseFare)        ? Number(pricing.baseFare)         : 0;
    let taxes            = isNonNegativeNumber(pricing.taxes)           ? Number(pricing.taxes)            : 0;
    let baggageFee       = isNonNegativeNumber(pricing.baggageFee)      ? Number(pricing.baggageFee)       : 0;
    let mealFee          = isNonNegativeNumber(pricing.mealFee)         ? Number(pricing.mealFee)          : 0;
    let seatSelectionFee = isNonNegativeNumber(pricing.seatSelectionFee)? Number(pricing.seatSelectionFee) : 0;
    let discount         = isNonNegativeNumber(pricing.discount)        ? Number(pricing.discount)         : 0;
...
module.exports = {
    ...
    generateUniquePNR,           /* async — needed by controllers */
    roundCurrency,
    calculateTaxes,
    calculateBaggageFee,
    calculateMealFee,
