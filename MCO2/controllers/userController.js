...
exports.getProfile = async (req, res) => {
    if (!req.session.user) {
        console.log('Profile retrieval failed: User not logged in.');
...
exports.updateProfile = async (req, res) => {
    if (!req.session.user) {
        console.log('Profile update failed: User not logged in.');
        return res.status(401).json({ success: false, message: 'Please login first.' });
    }
...
exports.getCurrentUser = async (req, res) => {
    if (!req.session.user) {
        console.log('Current user retrieval failed: User not logged in.');
        return res.status(401).json({ success: false, message: 'Please login first.' });
    }
    console.log('Current yser retrieved:', req.session.user.email);
